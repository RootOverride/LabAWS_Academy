...
instance_id1=$(aws ec2 run-instances --image-id $ami_id --instance-type t2.micro --key-name $email --security-group-ids $sg_id --subnet-id $subnet_id --user-data file://datosusuarioUbuntu1.txt --query 'Instances[*].InstanceId' --output text)
instance_id2=$(aws ec2 run-instances --image-id $ami_id --instance-type t2.micro --key-name $email --security-group-ids $sg_id --subnet-id $subnet_id --user-data file://datosusuarioUbuntu2.txt --query 'Instances[*].InstanceId' --output text)
...
aws ec2 associate-address --instance-id $instance_id1 --allocation-id $eip_allocation_id
