...
instance_id1=$(aws ec2 run-instances --image-id $ami_id --instance-type t2.micro --key-name $email --security-group-ids $sg_id --subnet-id $subnet_id --user-data file://setup1.sh --query 'Instances[*].InstanceId' --output text)
instance_id2=$(aws ec2 run-instances --image-id $ami_id --instance-type t2.micro --key-name $email --security-group-ids $sg_id --subnet-id $subnet_id --user-data file://setup2.sh --query 'Instances[*].InstanceId' --output text)
