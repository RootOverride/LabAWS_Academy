...
read -p "Ingrese el nombre de su Key Pair (por ejemplo: NombreApellido): " name1

# Validar si el Key Pair existe antes de intentar eliminarlo
if aws ec2 describe-key-pairs --key-name "$name1" > /dev/null 2>&1; then
  # Eliminar Key Pair
  aws ec2 delete-key-pair --key-name "$name1"
  echo "✅ Key Pair '$name1' eliminado."
else
  echo "❌ El Key Pair '$name1' no existe."
fi

# Obtener los IDs de las instancias asociadas
instance_ids=$(aws ec2 describe-instances --filters "Name=key-name,Values=$name1" --query "Reservations[*].Instances[*].InstanceId" --output text)
...
sg_id=$(aws ec2 describe-security-groups --filters "Name=group-name,Values=${name1}-sg" --query "SecurityGroups[0].GroupId" --output text)
...
if [ -n "$sg_id" ]; then
  ...
  echo "❌ No se encontró el Security Group '${name1}-sg'."
fi

# Obtener los IDs del Internet Gateway y la VPC
vpc_id=$(aws ec2 describe-vpcs --filters "Name=tag:Name,Values=$name1" --query 'Vpcs[0].VpcId' --output text)
...
echo "✅ Todos los recursos asociados con '$name1' han sido eliminados."
